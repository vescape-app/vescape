...
- **Plus one exception: how awake the Mirror is.** `MirrorAwake(level)` — `ASLEEP`/`ACTIVE`/`AMBIENT`, re-sent every 15 s with a 45 s phone-side dead-man, same shape as a Move hold. It is not rider intent, but it is not board state either: it is the wrist describing _itself_, which no other party can observe. The phone's capability gate only proves the Mirror is **installed**, so without this the phone pushed 4 Hz into a stopped activity for as long as its foreground service lived. Frames now flow only while the Mirror is on a wrist, at a cadence matching what that wrist can actually show. The gate is opt-in per wrist build via a second declared capability (`vescape_watch_mirror_wake`): phone and watch ship on separate Play tracks and update independently, so a phone that gated unconditionally would blank an older wrist that never sends a level.
