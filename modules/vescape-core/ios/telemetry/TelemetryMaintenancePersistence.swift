...
struct TelemetryMaintenancePersistence {
  ...
  func deleteRanges(_ ranges: [TelemetryTimeRange], boardId: String?, allBoards: Bool) throws -> Int {
    try writer.write { db in
      var count = 0
      for range in ranges {
        if allBoards {
  ...
  func rebuild(config: MetricSanitizerConfig, onProgress: (Int, Int) -> Void) throws -> Int {
    try writer.write { db in
      guard let first = try Int64.fetchOne(db, sql: "SELECT MIN(captured_at_ms) FROM telemetry_frames"),
            let last = try Int64.fetchOne(db, sql: "SELECT MAX(captured_at_ms) FROM telemetry_frames")
      else { return 0 }
      try db.execute(sql: "DELETE FROM telemetry_minute_buckets")
      try db.execute(sql: "DELETE FROM metric_exclusion_ranges")
      ...
      for index in 0..<chunks {
        ...
        for bucket in buildTelemetryBuckets(points) { try upsertBucket(db, bucket); rebuilt += 1 }
...
internal func bucketPoint(_ row: Row) -> BucketTelemetryPoint? {
  BucketTelemetryPoint(
    capturedAtMs: row["captured_at_ms"], boardId: row["board_id"],
    speedCentiKmh: row["speed_centi_kmh"] ?? 0, batteryVoltageMv: row["battery_voltage_mv"] ?? 0,
    motorCurrentMa: row["motor_current_ma"] ?? 0, batteryCurrentMa: row["battery_current_ma"] ?? 0,
    dutyPermille: row["duty_permille"] ?? 0, odometerCm: row["odometer_cm"],
    tempMosfetDeciC: row["temp_mosfet_deci_c"], tempMotorDeciC: row["temp_motor_deci_c"],
    gpsSpeedCentiMps: row["gps_speed_centi_mps"], gpsTimestampMs: row["location_timestamp_ms"],
    gpsAccuracyCm: row["accuracy_cm"], latitudeE7: row["latitude_e7"], longitudeE7: row["longitude_e7"],
    bearingCentiDeg: row["bearing_centi_deg"], altitudeCm: row["altitude_cm"],
